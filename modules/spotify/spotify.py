    ...
    def get_uri_spotify(self, artist_song=None, song=None, playlist=None):
        if artist_song and self.top_songs:
            for track in self.top_songs:
                ...
                track_uri = track[2]
                if song == None and (
                    artist_song.lower() in track_song
                    or artist_song.lower() in track_artist
                ):
                    print("found %s in top songs\n" % artist_song.title())
                    return track_uri
                if not song == None and artist_song.lower() in track_artist:
                    print(
                        "found %s by %s in top songs\n"
        ...
        try:
            if artist_song:
                ...
                results = sp.search(q=artist_song, limit=30, type="artist,track")
    ...
    def get_user_details(self):
        ...
        for playlist in playlists["items"]:
            self.user_playlists.append(
                [playlist["name"], playlist["uri"], playlist["tracks"]["total"]]
            )  # save ext url
    ...
    def grab_active_id(self, sp):
        ...
        try:
            if not self.devices["devices"] == []:
                ...
                with open(
                    self.path + "/resources/spotify.json", "w"
                ) as f:  # store to json
