...
class HeyDittoNet:
    ...
    def create_model(self):
        ...
        if self.model_type == 'HeyDittoNet-v2':
            ...
            T = 8  # number of LSTM time units
            CNN_OUT = 32  # number of CNN output channels
            LSTM_FEATURES = int(8*CNN_OUT)  # num of features per LSTM unit
            model = Sequential([
                layers.Input(shape=xshape),
                # layers.Resizing(32, 32),
                # layers.Normalization(),

                layers.Conv2D(24, (3, 3), strides=(1, 1),
                              padding="valid", activation="relu"),
                layers.BatchNormalization(),
                layers.MaxPooling2D(pool_size=(2, 2)),

                layers.Conv2D(28, (3, 3), strides=(2, 2),
